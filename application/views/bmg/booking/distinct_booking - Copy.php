              ...
              <div class="box">
                 <?php echo form_open_multipart('booking/update', array('id' => 'updateForm')); ?>
                    <div class="box-header">
                       <u><h4><?php echo $data->booking_name; ?></h4></u>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                       <table style="width: 65%;" class="table">
                          <tr>
                             ...
                             <td><input type="text" value="<?php echo $data->project_name; ?>" name="project_name" required></td>
                          </tr>
                          <tr>
                          ...
                          <tr>
                             ...
                             <td>
                                <?php echo date('F d, Y', strtotime($data->po_date)); ?>
                                <!--<input type="date" readonly value="<?php echo $data->po_date; ?>" name="po_date">-->
                            </td>
                          </tr>
                          <tr>
                             <td><label>Customer PO No.</label></td>
                             <td><input type="text" value="<?php echo $data->cust_po_num; ?>" name="cust_po_num" required></td>
                          </tr>
                          <tr>
                             ...
                             <td>
                                <?php
                                   if($data->uploaded_file != '') {
                                     ...
                                     echo '<a href="'.$path.'" target="_blank" title="'.$data->uploaded_file.'"><i class="fa fa-fw fa-file-pdf-o" style="color: #DD4B39;"></i></a>';
                                   } else {
                                     echo 'n/a';
                                   } 
                                   ?>
						  ...
						  <tr>
                             ...
                             <td>
								...
								<a href="<?php echo site_url('booking/?year='.$_GET['year']); ?>" class="btn">Back</a>
							 </td>
                          </tr>
                       </table>
                    </div>
         ...
         <script>        
            ...
            var obj = {
                ...
                colModel: [
                        ...
                        }, { title: "Price USD", dataType: "float", align: "right", dataIndx: "price", editable: true, width: 140,
							validations: [
								{ type: 'nonEmpty', msg: "Required" },
								{ type: 'gt', value: 0, msg: "better be > 1", warn: true}],
							render: function(ui) {
								var r = parseInt(ui.cellData).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
								r = r.split('.');
								if(r == 'NaN') {
									r = '';
								}
                        ...
                        { title: "Price <?php echo $data->currency; ?>", <?php echo $hidden_col; ?> dataType: "float", align: "right", dataIndx: "converted_price", editable: false, width: 140,
							render: function(ui) {
								var r = parseInt(ui.cellData).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
								r = r.split('.');
								if(r == 'NaN') {
                        ...
                        { title: "Discount (%)", dataType: "integer", align: "right", dataIndx: "discount", editable: true, width: 140,
							validations: [
						...
						{ 
                            title: "Amount <?php echo $data->currency; ?>", <?php echo $hidden_col; ?> dataType: "float", align: "right", dataIndx: "converted_total", editable: false, width: 180,
							render: function(ui) {
								var r = parseInt(ui.cellData).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
								r = r.split('.');
								if(r == 'NaN') {
