...
class Booking extends CI_Controller {
	...
	{				
		...
		if(isset($uri_crud)) {			
			...
			$message['update'][0] = 'dangsuccesser|Alert|Failed to update booking!';
			$message['update'][1] = '|Success|Successfully updated booking!';
	...
	public function get_specific()
	{
		...
		$this->load->view('template/header', $data);
		$this->load->view('booking/distinct_booking');
		$this->load->view('template/footer');
	}
	
	public function crud_option($booking_name = '', $rate = 0, $currency = 0) {
		if(isset($_POST['list'])) {	
			$dlist = $_POST['list'];
			
			if(isset($dlist["updateList"])) {
				$dlist["updateList"] = $this->updateList($dlist["updateList"], $rate, $currency);
			}

			if(isset($dlist["addList"])) {
				$dlist["addList"] = $this->addList($dlist["addList"], $booking_name, $rate, $currency);
			}		
		}
	}
	...
	public function addList($addList, $booking_name = '', $rate = 0, $currency = '')
	{		
		$product2=array();
		foreach($addList as $i => $value):
			...
			$product2["description"]=$addList[$i]["description"];            
			
			if($currency == 'USD') {				
				$price = $addList[$i]['price'];
				$product2["price"]=$addList[$i]["price"]; 
				$product2["converted_price"] = 0;      
				...
				$product2["converted_total"]=0;	           
			} else {
				$price = $addList[$i]['converted_price'];
				$product2["converted_price"]=$addList[$i]["converted_price"];
				if($currency == 'EUR') {
					$product2["price"]=round($addList[$i]["converted_price"] * (float) $rate, 2); 
					...
					$product2["total"]=round($total * (float) $rate, 2);	
					$product2["converted_total"]=$total;	
				} else {
					$product2["price"]=round($addList[$i]["converted_price"] / (float) $rate, 2); 	
	...
	public function updateList($updateList, $rate = 0, $currency = '')
	{
			...
			foreach($products as $i => $products2){
				if($products2["id"] == $id) {
					...
					if($currency == 'USD') {
						...
						$product2["converted_price"]=$product["converted_price"];

						if($currency == 'EUR') {
							$price = (float) $product["converted_price"]; 
							$product2["price"]=round($product["converted_price"] * (float) $rate, 2); 
	...
	{		
		if(isset($_POST['list'])) {	
			...
			{ 
				...
				$product2["converted_price"]=round($_GET["price"] * (float) $currency, 2);                 
				...
				$total = $t - $d;
				$product2["total"]=$total;			
				$product2["converted_total"]=round($total * (float) $currency, 2);			
	...
	{	
		...
		foreach($data as $key => $row) {			
			...
			$arr[$i]['product'] = $row->product; 
			$arr[$i]['description'] = $row->description; 
			$arr[$i]['price'] = round((float)$row->price, 2); 
			$arr[$i]['converted_price'] = round((float)$row->converted_price, 2); 
			$arr[$i]['qty'] = $row->qty;			
			$arr[$i]['discount'] = $row->discount;
			$arr[$i]['bill'] = round((float)$row->total, 2); 
			$arr[$i]['converted_total'] = round((float)$row->converted_total, 2); 
			$i++;
		}
		$json = json_encode($arr);
		$_SESSION['orders'] = $json;
